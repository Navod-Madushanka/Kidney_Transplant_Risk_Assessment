...
export default function NewPairPage() {
  ...
  async function handleRegister() {
    ...
    try {
      ...
      //
      // Restored after being deleted entirely (see
      // implementation-prompt-part-j.md J0-J3): the original version's
      // result auto-saved straight to antibody_profiles with no guard,
      // and could silently destroy an already-verified profile. The
      // backend now refuses that save whenever the patient already has
      // any antibody-profile rows (see ocr_job_service.py's
      // _save_bead_specificity_if_present) -- a no-op guard for THIS call
      // site specifically, since result.patientId is always a patient
      // just created moments ago and so can never have existing rows,
      // but it's what makes restoring this safe in general.

...
def calculate_cpra(
    ...
    # module docstring), so a sensitised patient's unacceptable-antigen list
    # can legitimately contain the same antigen name more than once. Combining
    # a repeated antigen's frequency into itself twice
    # (`2f - f^2` instead of `f`) overstated cPRA for essentially every
    # sensitised patient, independent of any OCR error -- this predates
    ...
    # normalised name before the union-probability combination is the
    # fix: cPRA answers "how sensitised is this patient against the
    # population," which only cares whether an antigen is present, not
    # how many source rows reported it.
    unique_antigens = list(dict.fromkeys(unacceptable_antigens))
    ...
    for antigen in unique_antigens:
        ...
        if frequency > 0.0:
            matched += 1
        # Union-probability combination -- assumes each unacceptable antigen
        # is an independent event. Real HLA loci are in linkage disequilibrium
    ...
    message = (
        "OK"
        if total == 0
        else f"{matched} of {total} unacceptable antigens matched the reference frequency table"
    )
